...
private:
    ...
    std::array<float, 410> createCriticInformation()
    {
        ...
        for (int player = 0; player < NUM_PLAYERS; player++)
        {
            std::copy(state.players.at((player + state.currentPlayer) % 4).revealedCards.begin(), state.players.at((player + state.currentPlayer) % 4).revealedCards.end(), fullInformation.begin() + baseIndex);
            baseIndex += 36;
        }
